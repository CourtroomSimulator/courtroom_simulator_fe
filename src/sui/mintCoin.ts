...
export async function mintCoin(address: string,amount:number): Promise<SuiTransactionBlockResponse> {
    ...
    const u64Option = tx.pure.option('u64', amount);  // 这里的100是你想传递的数字
    ...
    tx.moveCall({
        ...
        arguments: [tx.object(treasuryCapId), tx.pure.u64(amount), tx.pure.address(address)]
    });
    let result;
    try {
